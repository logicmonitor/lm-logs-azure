...
* (optional) Set `event_hub_name` variable to use a custom Event Hub name (defaults to `log-hub`)
...
* `eventHubName` (optional) - Event Hub name the Function listens to (default `log-hub`). Required when using a custom Event Hub name or when deploying the Function package directly; Terraform and ARM templates set this automatically.
